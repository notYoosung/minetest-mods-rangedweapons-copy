...
if minetest.global_exists("armor") and armor.attributes then table.insert(armor.attributes, "bullet_res") end
if minetest.global_exists("armor") and armor.attributes then table.insert(armor.attributes, "ammo_save") end
if minetest.global_exists("armor") and armor.attributes then table.insert(armor.attributes, "ranged_dmg") end

-- minetest.register_node("rangedweapons2:antigun_block", {
...
make_sparks = function(pos)
	minetest.sound_play("rengedweapons_ricochet", {
		pos,
		gain = 0.75
	})
	-- for i = 1, 9 do
...
rangedweapons_reload_gun = function(itemstack, player)
	...
	if GunCaps ~= nil then gun_unload_sound = GunCaps.gun_unload_sound or "" end

	minetest.sound_play(gun_unload_sound, {player})

	local gun_reload = 0.25

	if GunCaps ~= nil then gun_reload = GunCaps.gun_reload or 0.25 end
	...
	if GunCaps.suitable_ammo ~= nil then
		...
		for i = 1, inv:get_size("main") do
			...
			if player_has_ammo == 1 then break end
	...
	if player_has_ammo == 1 then
		...
		if GunCaps.gun_icon ~= nil then gun_icon = GunCaps.gun_icon end

		local ammo_icon = "rangedweapons_emergency_ammo_icon.png"

		if reload_ammo:get_definition().inventory_image ~= nil then ammo_icon = reload_ammo:get_definition().inventory_image end
		...
		if GunCaps.gun_magazine ~= nil then
			...
			if pos and dir and yaw then
				pos.y = pos.y + 1.4
				local obj = minetest.add_entity(pos, "rangedweapons2:mag")
				if obj then
					obj:set_properties({
						textures = {GunCaps.gun_magazine}
					})
					obj:set_velocity({
						x = dir.x * 2,
						y = dir.y * 2,
		...
		if GunCaps.gun_unloaded ~= nil then itemstack:set_name(GunCaps.gun_unloaded) end
...
rangedweapons_yeet = function(itemstack, player)
	...
	-- if minetest.find_node_near(player:getpos(), 10, "rangedweapons2:antigun_block") then
	...
	if ThrowCaps ~= nil then throw_cooldown = ThrowCaps.throw_cooldown or 0 end
	...
	if playerMeta:get_float("rw_cooldown") <= 0 then
		...
		local throw_ent = "rangedweapons2:shot_bullet"
		local throw_visual = "wielditem"
		local throw_texture = "rangedweapons2:shot_bullet_visual"
		...
		if ThrowCaps ~= nil then
			throw_damage = ThrowCaps.throw_damage or {
				fleshy = 1
			}
			...
			throw_ent = ThrowCaps.throw_entity or "rangedweapons2:shot_bullet"
			throw_visual = ThrowCaps.throw_visual or "wielditem"
			throw_texture = ThrowCaps.throw_texture or "rangedweapons2:shot_bullet_visual"
		...
		if throw_skillChance > 0 and throw_skill ~= "" then rangedweapons_gain_skill(player, throw_skill, throw_skillChance) end
		...
		rangedweapons_launch_projectile(player, throw_projectiles, throw_damage, throw_ent, throw_visual, throw_texture, throw_sound, throw_velocity, throw_accuracy, skill_value, OnCollision, throw_crit,
			throw_critEffc, throw_mobPen, throw_nodePen, 0, "", "", "", throw_dps, throw_gravity, throw_door_breaking, throw_glass_breaking, throw_particles, throw_sparks, throw_bomb_ignite, throw_size, 0,
...
rangedweapons_shoot_gun = function(itemstack, player)
	...
	if gunMeta:get_int("RW_bullets") > 0 and playerMeta:get_float("rw_cooldown") <= 0 then
		...
		if math.random(1, 100) > gun_ammo_save then gunMeta:set_int("RW_bullets", gunMeta:get_int("RW_bullets") - 1) end
		...
		local bulletStack = ItemStack({
			name = gunMeta:get_string("RW_ammo_name")
		})
		...
		local bullet_ent = "rangedweapons2:shot_bullet"
		local bullet_visual = "wielditem"
		local bullet_texture = "rangedweapons2:shot_bullet_visual"
		local bullet_crit = 0
		local bullet_critEffc = 0
		local bullet_projMult = 1
		local bullet_mobPen = 0
		local bullet_nodePen = 0
		local bullet_shell_ent = "rangedweapons2:empty_shell"
		local bullet_shell_visual = "wielditem"
		local bullet_shell_texture = "rangedweapons2:shelldrop"
		local bullet_dps = 0
		...
		if GunCaps ~= nil then
			gun_damage = GunCaps.gun_damage or {
				fleshy = 1
			}
		...
		if gun_skillChance > 0 and gun_skill ~= "" then rangedweapons_gain_skill(player, gun_skill, gun_skillChance) end

		local ammo_icon = "rangedweapons_emergency_ammo_icon.png"
		if bulletStack:get_definition().inventory_image ~= nil then ammo_icon = bulletStack:get_definition().inventory_image end
		player:hud_change(ammoimg, "text", ammo_icon)

		if AmmoCaps ~= nil then

			OnCollision = AmmoCaps.OnCollision or function() end
			bullet_damage = AmmoCaps.ammo_damage or {
				fleshy = 1
			}
			bullet_velocity = AmmoCaps.ammo_velocity or 0
			bullet_ent = AmmoCaps.ammo_entity or "rangedweapons2:shot_bullet"
			bullet_visual = AmmoCaps.ammo_visual or "wielditem"
			bullet_texture = AmmoCaps.ammo_texture or "rangedweapons2:shot_bullet_visual"
			...
			bullet_shell_ent = AmmoCaps.shell_entity or "rangedweapons2:empty_shell"
			bullet_shell_visual = AmmoCaps.shell_visual or "wielditem"
			bullet_shell_texture = AmmoCaps.shell_texture or "rangedweapons2:shelldrop"
		...
		for _, bulletDmg in pairs(bullet_damage) do if gun_damage[_] == nil then combined_dmg[_] = bullet_damage[_] end end
		...
		rangedweapons_launch_projectile(player, combined_projNum, combined_dmg, bullet_ent, bullet_visual, bullet_texture, gun_sound, combined_velocity, gun_accuracy, skill_value, OnCollision,
			combined_crit, combined_critEffc, combined_mobPen, combined_nodePen, gun_shell, bullet_shell_ent, bullet_shell_texture, bullet_shell_visual, combined_dps, combined_gravity, gun_door_breaking,
			bullet_glass_breaking, bullet_particles, bullet_sparks, bullet_bomb_ignite, bullet_size, gun_smokeSize, 0, bullet_glow)

		if rweapons_gun_durability == "true" then itemstack:add_wear(65535 / gun_durability) end
...
rangedweapons_shoot_powergun = function(itemstack, player)
	...
	if inv:contains_item("main", "rangedweapons2:power_particle " .. PowerCaps.power_consumption) and playerMeta:get_float("rw_cooldown") <= 0 then
		...
		local power_ent = "rangedweapons2:shot_bullet"
		local power_visual = "wielditem"
		local power_texture = "rangedweapons2:shot_bullet_visual"
		...
		if PowerCaps ~= nil then
			power_damage = PowerCaps.power_damage or {
				fleshy = 1
			}
			...
			power_ent = PowerCaps.power_entity or "rangedweapons2:shot_bullet"
			power_visual = PowerCaps.power_visual or "wielditem"
			power_texture = PowerCaps.power_texture or "rangedweapons2:shot_bullet_visual"
		...
		if power_skillChance > 0 and power_skill ~= "" then rangedweapons_gain_skill(player, power_skill, power_skillChance) end
		...
		rangedweapons_launch_projectile(player, power_projectiles, power_damage, power_ent, power_visual, power_texture, power_sound, power_velocity, power_accuracy, skill_value, OnCollision, power_crit,
			power_critEffc, power_mobPen, power_nodePen, 0, "", "", "", power_dps, power_gravity, power_door_breaking, power_glass_breaking, power_particles, power_sparks, power_bomb_ignite, power_size, 0, 0,
			power_glow)

		if rweapons_gun_durability == "true" then itemstack:add_wear(65535 / power_durability) end
		itemstack:set_name(power_cooling)

		inv:remove_item("main", "rangedweapons2:power_particle " .. PowerCaps.power_consumption)

	end
	-- end
end

rangedweapons_launch_projectile = function(player, projNum, projDmg, projEnt, visualType, texture, shoot_sound, combined_velocity, accuracy, skill_value, ColResult, projCrit, projCritEffc, mobPen,
	nodePen, has_shell, shellEnt, shellTexture, shellVisual, dps, gravity, door_break, glass_break, bullet_particles, sparks, ignite, size, smokeSize, proj_wear, proj_glow)
	...
	if pos and dir and yaw then
		...
		for i = 1, projectiles do
			...
			if obj then
				...
				ent.wear = proj_wear
				obj:set_velocity({
					x = dir.x * combined_velocity,
					y = dir.y * combined_velocity,
					z = dir.z * combined_velocity
				})
				acc = (((100 - accuracy) / 10) / skill_value) or 0
				obj:set_acceleration({
					x = math.random(-acc, acc),
					y = math.random(-acc, acc) - gravity,
...
eject_shell = function(itemstack, player, rld_item, rld_time, rldsound, shell)
	...
	if pos and dir and yaw then
		pos.y = pos.y + 1.6
		local obj = minetest.add_entity(pos, "rangedweapons2:empty_shell")
...
if rweapons_shurikens == "true" then dofile(modpath .. "/shurikens.lua") end
...
if rweapons_forceguns == "true" then dofile(modpath .. "/forcegun.lua") end

if rweapons_javelins == "true" then dofile(modpath .. "/javelin.lua") end
...
if rweapons_explosives == "true" then
	...
	dofile(modpath .. "/rocker.lua")
end

if rweapons_glass_breaking == "true" then dofile(modpath .. "/glass_breaking.lua") end
if not minetest.settings:get_bool("glass_breaking") then
	minetest.register_abm({
		nodenames = {"rangedweapons2:broken_glass"},
		interval = 1,
		chance = 1,
		action = function(pos, node)
			if minetest.get_node(pos).name == "rangedweapons2:broken_glass" then
...
local rangedweapons_empty_shell = {
	...
	textures = {"rangedweapons2:shelldrop"},
	lastpos = {},
...
rangedweapons_empty_shell.on_step = function(self, dtime, pos)
	...
	if self.lastpos.y ~= nil then
		if minetest.registered_nodes[node.name] ~= nil then
			if minetest.registered_nodes[node.name].walkable then
				...
				minetest.sound_play("rangedweapons_shellhit", {
					pos = self.lastpos,
					gain = 0.8
				})
	...
	if self.timer > 1.69 then
		minetest.sound_play("rangedweapons_bulletdrop", {
			pos = self.lastpos,
			gain = 0.8
		})
		self.object:remove()
...
minetest.register_entity("rangedweapons2:empty_shell", rangedweapons_empty_shell)
