...
minetest.register_craftitem("rangedweapons2:thrown_javelin", {
	wield_scale = {x=2.0,y=2.0,z=1.0},
	inventory_image = "rangedweapons_thrown_javelin.png",
})

minetest.register_craftitem("rangedweapons2:javelin", {
	...
	on_secondary_use = function(itemstack, user, pointed_thing)
		...
		if pos and dir then
			pos.y = pos.y + 1.5
			local obj = minetest.add_entity(pos, "rangedweapons2:javelin")
			if obj then
...
local rangedweapons_javelin = {
	...
	textures = {"rangedweapons2:thrown_javelin"},
	lastpos= {},
	...
	if self.timer > 0.15 then
		...
		for k, obj in pairs(objs) do
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "rangedweapons2:javelin" and obj:get_luaentity().name ~= "__builtin:item" then
			...
			if not minetest.setting_getbool("creative_mode") and math.random(1, 100) <= 95 then
				minetest.add_item(self.lastpos, "rangedweapons2:javelin")
			end
					else
...
minetest.register_entity("rangedweapons2:javelin", rangedweapons_javelin)
