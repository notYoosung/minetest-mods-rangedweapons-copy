minetest.register_tool("rangedweapons2:forcegun", {
...
if  inv:contains_item("main", "rangedweapons2:power_particle 40") then
		if pos and dir then
		inv:remove_item("main", "rangedweapons2:power_particle 25")
			pos.y = pos.y + 1.5
			local obj = minetest.add_entity(pos, "rangedweapons2:forceblast")
			if obj then
...
if posd_y > -1.0 and posd_y < 0 then posd_y = -1.0 end
			...
			if obj:get_luaentity() ~= nil then
				if obj:get_luaentity().name ~= "rangedweapons2:forceblast" then
...
minetest.register_entity("rangedweapons2:forceblast", rangedweapons_forceblast)
